...
public enum SDPhotoAlbumUtilResult {
    case success, error, denied
}

// MARK:- 一、一、基本的使用
public class SDPhotoAlbumUtil: NSObject {
    ...
    ///   - completion: 结果闭包
    public class func saveImageInAlbum(image: UIImage, isCustomPhotoAlbumName: Bool = false, completion: ((SDPhotoAlbumUtilResult) -> Void)?) {
        ...
        if !isCustomPhotoAlbumName {
            ...
            // 相册的名字
            let albumName = Bundle.sd.bundleName
            // 看保存的指定相册是否存在
