...
public class SDWeakTimer {
    ...
    private var timerCallBack: ((SDWeakTimer) -> Void)?
    ...
    ///   - yesOrNo: 是否重复 默认 false
    /// - Returns: 已启动的timer
    public class func scheduledTimer(timeInterval ti: TimeInterval = 1.0, target aTarget: NSObjectProtocol, selector aSelector: Selector, userInfo: Any? = nil, repeats yesOrNo: Bool = false) -> SDWeakTimer {
        let timer = SDWeakTimer.init()
    ...
    ///   - block: 回调
    public convenience init(timeInterval interval: TimeInterval = 1.0, repeats: Bool = false, queue: DispatchQueue? = nil, block: @escaping (SDWeakTimer) -> Void) {
        self.init()
    ...
    ///   - block: 回调
    /// - Returns: 已启动的timer
    public class func scheduledTimer(timeInterval interval: TimeInterval = 1.0, repeats: Bool = false, queue: DispatchQueue? = nil, block: @escaping (SDWeakTimer) -> Void) -> SDWeakTimer {
        let timer = SDWeakTimer.init()
    ...
    ///   - queue: 调用队列 默认 nil 全局队列
    ///   - block: 回调
    public convenience init(fire date: Date, interval: TimeInterval = 1.0, repeats: Bool, queue: DispatchQueue? = nil, block: @escaping (SDWeakTimer) -> Void) {
        self.init()
        _fireDate = date
        timeInterval = interval
        isRepeats = repeats
