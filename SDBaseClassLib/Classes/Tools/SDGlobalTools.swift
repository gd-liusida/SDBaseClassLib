...
public struct SDGlobalTools {
    ...
    public static func compareVersion(version: String) -> Bool {
        ...
        // 2、当前版本的版本号获取 三位Int值
        let currentVersion = Bundle.sd.appVersion
    ...
    public static func openUrl(url: URL, complete: @escaping ((Bool) -> Void)) {
        ...
        UIApplication.shared.open(url, options: [:]) { (success) in
            if (success) {
                SDTool.log("10以后可以跳转url")
                complete(true)
            } else {
                SDTool.log("10以后不能完成跳转")
                complete(false)
            }
        }
    }
...
private extension SDGlobalTools {
    ...
    static func appVersion(version: String) -> (isSuccess: Bool, versions: [Int]) {
        ...
        guard let versionValue1 = versionString1.sd.toInt(), let versionValue2 = versionString2.sd.toInt(), let versionValue3 = versionString3.sd.toInt() else {
            return (false, [])
        }
