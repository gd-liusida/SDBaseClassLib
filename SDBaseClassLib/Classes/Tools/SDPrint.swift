...
public func SDPrint(_ msg: Any...,
    ...
    let prefix = "---begin---------------🚀----------------\n当前时间：\(Date.sd.currentDate)\n当前文件完整的路径是：\(file)\n当前文件是：\(file.lastPathComponent)\n第 \(line) 行 \n第 \(column) 列 \n函数名：\(fn)\n打印内容如下：\n\(msgStr)---end-----------------😊----------------"
    print(prefix)
    // 将内容同步写到文件中去（Caches文件夹下）
    let cachePath = FileManager.sd.CachesDirectory()
...
private func appendText(fileURL: URL, string: String) {
    do {
        // 如果文件不存在则新建一个
        FileManager.sd.createFile(filePath: fileURL.path)
