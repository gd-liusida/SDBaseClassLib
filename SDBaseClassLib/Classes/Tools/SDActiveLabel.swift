...
public protocol SDActiveLabelDelegate: class {
    func didSelect(_ text: String, type: SDActiveType)
}

public typealias SDConfigureLinkAttribute = (SDActiveType, [NSAttributedString.Key : Any], Bool) -> ([NSAttributedString.Key : Any])
typealias SDElementTuple = (range: NSRange, element: SDActiveElement, type: SDActiveType)


@IBDesignable open class SDActiveLabel: UILabel {

    // MARK: - public properties
    open weak var delegate: SDActiveLabelDelegate?
    
    open var enabledTypes: [SDActiveType] = [.mention, .hashtag, .url]
    
    open var urlMaximumLength: Int?
    
    open var configureLinkAttribute: SDConfigureLinkAttribute?
    ...
    open var customColor: [SDActiveType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    open var customSelectedColor: [SDActiveType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: SDActiveType, handler: @escaping (String) -> ()) {
    ...
    open func removeHandle(for type: SDActiveType) {
        switch type {
        case .hashtag:
    ...
    // MARK: - customzation
    @discardableResult
    open func customize(_ block: (_ label: SDActiveLabel) -> ()) -> SDActiveLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [SDActiveType : ((String) -> ())] = [:]
    
    fileprivate var mentionFilterPredicate: ((String) -> Bool)?
    fileprivate var hashtagFilterPredicate: ((String) -> Bool)?
    
    fileprivate var selectedElement: SDElementTuple?
    fileprivate var heightCorrection: CGFloat = 0
    internal lazy var textStorage = NSTextStorage()
    fileprivate lazy var layoutManager = NSLayoutManager()
    fileprivate lazy var textContainer = NSTextContainer()
    lazy var activeElements = [SDActiveType: [SDElementTuple]]()
    ...
    fileprivate func parseTextAndExtractActiveElements(_ attrString: NSAttributedString) -> String {
        ...
        if enabledTypes.contains(.url) {
            let tuple = SDActiveBuilder.createURLElements(from: textString, range: textRange, maximumLength: urlMaximumLength)
        ...
        for type in enabledTypes where type != .url {
            ...
            let hashtagElements = SDActiveBuilder.createElements(type: type, from: textString, range: textRange, filterPredicate: filter)
    ...
    fileprivate func element(at location: CGPoint) -> SDElementTuple? {
    ...
    fileprivate func didTap(_ element: String, for type: SDActiveType) {
...
extension SDActiveLabel: UIGestureRecognizerDelegate {
