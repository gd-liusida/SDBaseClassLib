...
extension UIImage: SDPOPCompatible {}

// MARK:- 一、基本的扩展
public extension SDPOP where Base: UIImage {
    ...
    static func textImage(_ text: String, fontSize: CGFloat = 16, size: (CGFloat, CGFloat), backgroundColor: UIColor = UIColor.orange, textColor: UIColor = UIColor.white, isCircle: Bool = true, isFirstChar: Bool = false) -> UIImage? {
        ...
        let textsize = text.sd.rectSize(font: UIFont.systemFont(ofSize: fontSize), size: CGSize(width: kScreenW, height: CGFloat(MAXFLOAT)))
    ...
    func solidTo(size: CGSize) -> UIImage? {
        ...
        if self.base.size.height <= self.base.size.width {
            if self.base.size.width >= w {
                let scaleImage = fixOrientation().sd.scaleTo(size: CGSize(width: w, height: w * self.base.size.height / self.base.size.width))
                return scaleImage
            } else {
            ...
            if self.base.size.height >= h {
                let scaleImage = fixOrientation().sd.scaleTo(size: CGSize(width: h * self.base.size.width / self.base.size.height, height: h))
                return scaleImage
            } else {
                return fixOrientation()
            }
...
public extension SDPOP where Base: UIImage {
    ...
    func compress(mode: CompressionMode = .medium) -> Data? {
        return resizeIO(resizeSize: mode.resize(base.size))?.sd.compressDataSize(maxSize: mode.maxDataSize)
    }
    ...
    func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@escaping (Data?, CGSize) -> Void) {
        queue.async {
            let data = resizeIO(resizeSize: mode.resize(self.base.size))?.sd.compressDataSize(maxSize: mode.maxDataSize)
            DispatchQueue.main.async {
...
public extension SDPOP where Base: UIImage {
    ...
    private static func getHDImgWithCIImage(with image: CIImage, size: CGSize, isLogo: Bool = true, logoSize: CGSize?, logoImage: UIImage? = nil, logoRoundCorner: CGFloat? = nil) -> UIImage? {
        ...
        if let newLogoRoundCorner = logoRoundCorner, let roundCornerLogo = logoImage.sd.isRoundCorner(radius: newLogoRoundCorner, byRoundingCorners: .allCorners, imageSize: logoSize) {
...
public extension SDPOP where Base: UIImage {
    ...
    static func gif(data: Data) -> UIImage? {
        ...
        guard let source = CGImageSourceCreateWithData(data as CFData, nil) else {
            SDPrint("SwiftGif: Source for the image does not exist")
            return nil
        }
    ...
    static func gif(url: String) -> UIImage? {
        ...
        guard let bundleURL = URL(string: url) else {
            SDPrint("SwiftGif: This image named \"\(url)\" does not exist")
            return nil
        }
        // Validate data
        guard let imageData = try? Data(contentsOf: bundleURL) else {
            SDPrint("SwiftGif: Cannot turn image named \"\(url)\" into NSData")
            return nil
        }
    ...
    static func gif(name: String) -> UIImage? {
        ...
        guard let bundleURL = Bundle.main
            ...
            SDPrint("SwiftGif: This image named \"\(name)\" does not exist")
            return nil
        }
        // Validate data
        guard let imageData = try? Data(contentsOf: bundleURL) else {
            SDPrint("SwiftGif: Cannot turn image named \"\(name)\" into NSData")
            return nil
        }
    ...
    static func gif(asset: String) -> UIImage? {
        ...
        guard let dataAsset = NSDataAsset(name: asset) else {
            SDPrint("SwiftGif: Cannot turn image named \"\(asset)\" into NSDataAsset")
            return nil
        }
