...
extension UIView: SDPOPCompatible {}
...
public extension SDPOP where Base: UIView {
    // MARK: 3.1、x 的位置
    /// x 的位置
    var x: CGFloat {
        get {
...
/**
 ...
 m44:初始为1
 */
extension SDPOP where Base: UIView {
...
public extension SDPOP where Base: UIView {
    ...
    func drawCircle(fillColor: UIColor, strokeColor: UIColor, strokeWidth: CGFloat) {
        let ciecleRadius = self.base.sd.width > self.base.sd.height ? self.base.sd.height : self.base.sd.width
...
// MARK:- 七、其他的方法
// 抖动方向枚举
public enum SDShakeDirection: Int {
...
public extension SDPOP where Base: UIView {
    ...
    func addWater(markText: String, textColor: UIColor = UIColor.black, font: UIFont = UIFont.systemFont(ofSize: 12)) {
        let waterMark: NSString = markText.sd.toNSString
        let textSize: CGSize = waterMark.size(withAttributes: [NSAttributedString.Key.font : font])
        // 多少行
        let line: NSInteger = NSInteger(self.base.sd.height * 3.5 / 80)
        // 多少列：自己的宽度/(每个水印的宽度+间隔)
        let row: NSInteger = NSInteger(self.base.sd.width / markText.sd.rectWidth(font: font, size: CGSize(width: self.base.sd.width, height: CGFloat(MAXFLOAT))))
        for i in 0..<line {
            for j in 0..<row {
    ...
    func shake(direction: SDShakeDirection = .horizontal, times: Int = 3, interval: TimeInterval = 0.1, delta: CGFloat = 2, completion: (() -> Void)? = nil) {
        // 播放动画
...
public extension SDPOP where Base: UIView {
    ...
    func getViewLayer(borderWidth: CGFloat = 0.5, borderColor: UIColor = .randomColor, backgroundColor: UIColor = .randomColor) {
        ...
        for subview in subviews {
            ...
            subview.sd.getViewLayer(borderWidth: borderWidth, borderColor: borderColor, backgroundColor: backgroundColor)
        }
        #endif
    }
    ...
    func findSubview(type: UIResponder.Type, resursion: Bool)-> UIView? {
        ...
        for e in self.base.subviews.enumerated() {
            let tmpView = e.element.sd.findSubview(type: type, resursion: resursion)
            if tmpView != nil {
                return tmpView
            }
        }
...
public extension SDPOP where Base: UIView {
    ...
    func addActionClosure(_ actionClosure: @escaping ViewClosure) {
        if let sender = self.base as? UIButton {
            sender.sd.setHandleClick(controlEvents: .touchUpInside) { (control) in
            ...
            sender.sd.addActionHandler({ (control) in
                actionClosure(nil, control, control.tag)
            }, for: .valueChanged)
        } else {
            _ = self.base.sd.addGestureTap { (reco) in
