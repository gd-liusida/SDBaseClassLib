...
import Foundation
// MARK:- 一、基本的扩展
public extension SDPOP where Base: UINavigationController {
    ...
    func pop(count: Int, animated: Bool) {
        guard count >= 1 else {
            SDPrint("can't pop count less 1")
            return
        }
        let vcsCount = base.viewControllers.count
        if count >= vcsCount {
            SDPrint("count: \(count), must less than viewControllers count: \(vcsCount); will pop to root now!")
