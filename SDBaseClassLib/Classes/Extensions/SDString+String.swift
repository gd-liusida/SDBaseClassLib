...
public extension String {
    ...
    // MARK: 1.13 判断是否包含某个子串
    /// 判断是否包含某个子串
    func contains(find: String) -> Bool {
        return self.range(of: find) != nil
    }
    
    // MARK: 1.14、判断是否包含某个子串 -- 忽略大小写
    /// 判断是否包含某个子串 -- 忽略大小写
    func containsIgnoringCase(find: String) -> Bool {
        return self.range(of: find, options: .caseInsensitive) != nil
    }
    
    // MARK: 1.15、判断是不是九宫格键盘
    /// 判断是不是九宫格键盘
    ...
    // MARK: 1.16、字符串转 UIViewController
...
public extension String {
    ...
    ///   - height: 宽度
    /// - Returns: 返回宽度
    func autoAttStringWidth(_ font: UIFont, height: CGFloat = UIScreen.main.bounds.height) -> CGFloat {
        ...
        let str: NSString = self as NSString
        let size = str.boundingRect(with: CGSize.init(width: CGFloat(MAXFLOAT),
                                                      height: height),
                                    options: NSStringDrawingOptions.usesLineFragmentOrigin,
                                    attributes: att,
                                    context: nil).size
    ...
    // MARK: 4.5、对字符串(多行)指定出字体大小和最大的 Size，获取 (Size)
...
extension String {
    ...
    // MARK: 11.3、将编码后的url转换回原始的url
    /// 将编码后的url转换回原始的url
    var urlDecoded: String? {
        return self.removingPercentEncoding
    }
    
    // MARK: 11.4、url编码 会对所有特殊字符做编码  特殊情况下使用
...
public extension String {
    ...
    func md5Encrypt(_ md5Type: MD5EncryptType = .lowercase32) -> String {
        guard self.count > 0 else {
            SDTool.log("⚠️⚠️⚠️md5加密无效的字符串⚠️⚠️⚠️")
            return ""
        }
