...
extension CALayer: SDPOPCompatible {}
...
public extension SDPOP where Base: CALayer {
...
public extension SDPOP where Base: CALayer {
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func addKeyframeAnimationRotation(values: [Any] = [CGFloat(-5).sd.degreesToRadians(), CGFloat(5).sd.degreesToRadians(), CGFloat(-5).sd.degreesToRadians()],
                                      keyTimes: [NSNumber]?,
