...
extension UITextView {
    ...
    fileprivate func initPlaceholder(_ placeholder: String) {
        ...
        let rect = placeholder.boundingRect(with: CGSize(width: self.sd.width - placeholderOrigin!.x, height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : self.placeholdFont!], context: nil)
        placeholderLabel?.frame = CGRect(x: placeholderOrigin!.x + 1, y: placeholderOrigin!.y, width: self.sd.width - placeholderOrigin!.x * 2 - 1, height: rect.size.height)
    ...
    @objc fileprivate func textChange(_ notification : Notification) {
        ...
        if autoHeight == true {
            ...
            UIView.animate(withDuration: 0.15) { [weak self] in
                ...
                weakSelf.frame = CGRect(x: weakSelf.sd.x, y: weakSelf.sd.y, width: weakSelf.sd.width, height: size.height + weakSelf.placeholderOrigin!.y * 2 < weakSelf.oldFrame!.height ? weakSelf.oldFrame!.height : size.height + weakSelf.placeholderOrigin!.y * 2)
            }
        }
    }
...
// MARK:- 二、文本链接的扩展
public extension SDPOP where Base: UITextView {
    ...
    func resolveHashTags() {
        ...
        for sentence in sentences {
            // 如果是url链接则跳过
            if !(sentence as String).sd.verifyUrl() {
                // 再按特殊符号拆分
    ...
    func inputRestrictions(shouldChangeTextIn range: NSRange, replacementText text: String, maxCharacters: Int, regex: String?) -> Bool {
        ...
        if let _ = self.base.markedTextRange {
            ...
            if range.length == 0 {
                ...
                // 正则的判断
                if let weakRegex = regex, SDRegexHelper.match(text, pattern: weakRegex) {
                    return false
                }
                // 联想选中键盘
                let allContent = oldContent.sd.sub(to: range.location) + text
                if allContent.count > maxCharacters  {
                    let newContent = allContent.sd.sub(to: maxCharacters)
            ...
            guard !text.sd.isNineKeyBoard() else {
                return true
            }
            // 正则的判断
            if let weakRegex = regex, SDRegexHelper.match(text, pattern: weakRegex) {
