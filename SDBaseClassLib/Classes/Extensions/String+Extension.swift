...
// MARK:- 一：字符串基本的扩展
public extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    func toClass() -> AnyClass? {
        ...
        guard let Class: AnyClass = NSClassFromString(namespace.sd.removeSomeStringUseSomeString(removeString: " ", replacingString: "_") + "." + (base as! String)) else {
            return nil
        }
        return Class
    }
...
public extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    var toNumber: NSNumber? {
        return self.toDouble()?.sd.number
    }
}
...
public extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    func accuraterDouble() -> Double? {
        ...
        SDPrint(NSDecimalNumber(decimal: decimal).doubleValue)
...
extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    public var isValidIDCardNumStrict: Bool {
        ...
        case 15:
            ...
            year = Int(str.sd.sub(start: 6, length: 2))!
        ...
        case 18:
            // 18位身份证
            year = Int(str.sd.sub(start: 6, length: 4))!
            ...
            if numberOfMatch > 0 {
                ...
                for i in 0 ..< 17 {
                    if let d = Int(str.sd.slice(i ..< (i + 1))) {
                        s += d * jiaoYan[i % 10]
                    } else {
                        return false
                    }
                ...
                let Y = s % 11
                let JYM = "10X98765432"
                let M = JYM.sd.sub(start: Y, length: 1)
                if M == str.sd.sub(start: 17, length: 1) {
                    return true
                } else {
                    return false
                }
...
extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    public func nsRange(of subString: String) -> NSRange? {
        guard (base as! String).sd.contains(find: subString) else {
            return nil
        }
    ...
    public func insertString(content: String, locat: Int) -> String {
        ...
        let str1 = (base as! String).sd.sub(to: locat)
        let str2 =  (base as! String).sd.sub(from: locat + 1)
...
public extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    func binaryToOctal() -> String {
        ...
        // 八进制
        return decimal.sd.decimalToOctal()
    }
    ...
    func binaryToHexadecimal() -> String {
        ...
        let decimal = binary.binaryTodecimal()
        // 十六进制
        return decimal.sd.decimalToHexadecimal()
    }
    ...
    func octalTobinary() -> String {
        ...
        // 二进制
        return decimal.sd.decimalToBinary()
    }
    ...
    func octalToHexadecimal() -> String {
        ...
        // 十进制
        let decimal = octal.sd.octalTodecimal()
        // 十六进制
        return decimal.sd.decimalToHexadecimal()
    }
    ...
    func decimalToBinary() -> String {
        guard var decimal = (self.base as! String).sd.toInt() else {
            return ""
        }
        var str = ""
        while decimal > 0 {
    ...
    func decimalToOctal() -> String {
        guard let decimal = (self.base as! String).sd.toInt() else {
            return ""
        }
        /*
...
public extension SDPOP where Base: ExpressibleByStringLiteral {
    ...
    func md5Encrypt(_ md5Type: MD5EncryptType = .lowercase32) -> String {
        guard (self.base as! String).count > 0 else {
            SDPrint("⚠️⚠️⚠️md5加密无效的字符串⚠️⚠️⚠️")
            return ""
        }
...
public extension SDPOP where Base == String {
