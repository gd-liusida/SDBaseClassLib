...
extension Date: SDPOPCompatible {}

/// 时间戳的类型
public enum SDTimestampType: Int {
    // 秒
    case second
    // 毫秒
    case millisecond
...
// MARK:- 一、Date 基本的扩展
public extension SDPOP where Base == Date {
...
public extension SDPOP where Base == Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampDouble = timestamp.sd.toDouble() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
    ...
    /// - Returns: 返回转化后的时间戳
    static func formatterTimeStringToTimestamp(timesString: String, formatter: String, timestampType: SDTimestampType = .second) -> String {
...
public extension SDPOP where Base == Date {
    ...
    var isYesterday: Bool {
        // 1.先拿到昨天的 date
        guard let date = Base().sd.yesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
    ...
    var isTheDayBeforeYesterday: Bool  {
        // 1.先拿到前天的 date
        guard let date = Base().sd.theDayBeforYesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
