...
extension UIApplication: SDPOPCompatible {}
// MARK:- 一、基本的扩展
public extension SDPOP where Base: UIApplication {
    ...
    static var userAgent: String {
        ...
        return "JK" + Bundle.sd.appVersion
    }
    
    // MARK: 1.4、app定位区域
    /// app定位区域
    static var localizations: String? {
        guard let weakInfoDictionary = Bundle.sd.infoDictionary, let content = weakInfoDictionary[String(kCFBundleLocalizationsKey)] else {
            return nil
        }
...
// MARK:- 二、APP权限的检测
public enum SDAppPermissionType {
    // 照相机
    case camera
    // 相册
    case album
...
public extension SDPOP where Base: UIApplication {
    ...
    /// - Returns: 结果
    static func isOpenPermission(_ permission: SDAppPermissionType) -> Bool {
...
/// 系统app
public enum SDSystemAppType: String {
    case safari = "http://"
...
/// 第三方app
public enum SDThirdPartyAppType: String {
    /// 微信
    case weixin = "weixin://"
    /// QQ
    case qq = "mqq://"
...
public extension SDPOP where Base: UIApplication {
    ...
    static func openSystemApp(type: SDSystemAppType, complete: @escaping ((Bool) -> Void)) {
        SDGlobalTools.openUrl(url: URL(string: type.rawValue)!, complete: complete)
    }
    
    // MARK: 打开第三方app
    /// 打开第三方app
    /// - Parameter type: 第三方app类型
    static func openThirdPartyApp(type: SDThirdPartyAppType, complete: @escaping ((Bool) -> Void)) {
        SDGlobalTools.openUrl(url: URL(string: type.rawValue)!, complete: complete)
    }
}
