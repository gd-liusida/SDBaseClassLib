...
extension FileManager: SDPOPCompatible {}
// MARK:- 一、沙盒路径的获取
/*
 - 1、Home(应用程序包)目录
...
public extension SDPOP where Base: FileManager {
...
public extension SDPOP where Base: FileManager {
    ...
    static func readFromFile(readType: FileWriteType, readPath: String) -> (isSuccess: Bool, content: Any?, error: String) {
        ...
        case .ArrayType:
            ...
            return (true, readString.sd.jsonStringToArray(), "")
        case .DictionaryType:
            guard let readString = String(data: data, encoding: String.Encoding.utf8) else {
                return (false, nil, "读取内容失败")
            }
            return (true, readString.sd.jsonStringToDictionary(), "")
        }
    }
...
public extension SDPOP where Base: FileManager {
    ...
    /// - Returns: 返回获取的图片
    private static func getVideoImage(videoUrlSouceType: SDVideoUrlType = .local, path: String, seconds: Double = 1, preferredTimescale: CMTimeScale = 10, maximumSize: CGSize?, preferredTrackTransform: Bool = true) -> UIImage? {
        var videoURL: URL?
