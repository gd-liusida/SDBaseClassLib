...
import Foundation
import AVFoundation
extension AVAssetExportSession: SDPOPCompatible { }
// MARK:- 一、有关视频压缩的扩展
public extension SDPOP where Base: AVAssetExportSession {
    // MARK: 1.1、本地视频压缩
    /// 本地视频压缩
    /// - Parameters:
    ///   - inputPath: 输入视频路径
    ...
    static func assetExportSession(inputPath: String, outputPath: String, outputFileType: AVFileType = .mp4, completionHandler handler: @escaping (AVAssetExportSession, Float64, String, String) -> Void, shouldOptimizeForNetworkUse: Bool = true, exportPresetMediumQuality: String = AVAssetExportPresetMediumQuality) {
        // 1、先检查是否存在输入是视频路径
        guard FileManager.sd.judgeFileOrFolderExists(filePath: inputPath) else {
            return
        }
        // 2、先移除转换后的路径（否则无法导出视频）
        FileManager.sd.removefile(filePath: outputPath)
        // 3、获取视频资源
        ...
        exportSession.exportAsynchronously {
            DispatchQueue.main.async {
                if exportSession.status == .completed {
                    handler(exportSession, duration, FileManager.sd.fileOrDirectorySize(path: outputPath), outputPath)
                } else {
